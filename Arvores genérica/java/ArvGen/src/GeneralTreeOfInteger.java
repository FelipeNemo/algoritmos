...
public class GeneralTreeOfInteger {
    ...
    private Node searchNodeRef(Integer elem, Node n) { // coloco o elemento e o Nodo n(raiz) 
        if (n == null)                                 // Então eu vou passar 3 vezes por ser uma chamada recursiva:
            return null;                               // sr("f", A); sr("f", B), sr("f", f);
